...
class AlibabaSpider(scrapy.Spider):
	...
	def parse(self, response):
		...
		logDatasNum = cursor.execute(sql_getLog)
		...
		if(logDatasNum != 0):
			...
			cursor.execute(sql_addNewLog, [AlibabaSpider.dataDateTime,'alibaba',AlibabaSpider.nowDateTime,AlibabaSpider.start_urls[0]])
			connect.commit()

			AlibabaSpider.entry_log_id = cursor.lastrowid
		...
		AlibabaSpider.allLocationDatas = list(cursor.fetchall())
		...
		for province in provinceArray:
			...
			if(provinceDataInDb_matrix.empty):
				...
				cursor.execute(sql_addNewArea_province, [2,1,provinceName])
				connect.commit()

				province_id = int(cursor.lastrowid)
			...
			for city in cityArray:
				...
				if(cityDataInDb_matrix.empty):
					...
					cursor.execute(sql_addNewArea_city, [3,province_id,cityName])
					connect.commit()

					city_id = int(cursor.lastrowid)
		...
		for country in abroadCountry:
			...
			if(countryDataInDb_matrix.empty):
				...
				cursor.execute(sql_addNewArea_country, [1,None,countryName])
				connect.commit()
		...
		# print(AlibabaSpider.newDatasToDb)

		# 关闭数据库连接
		cursor.close()
		connect.close()

	def addNewTo_newDatasToDb(newRecord,location_id = None,country_id = None,province_id = None,city_id = None):
